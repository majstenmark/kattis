...
def segment_intersect(s1, s2):
    ...
    res = sign(d1) != sign(d2) and sign(d3) != sign(d4)
    if d1 * d2 * d3 * d4 == 0 or res:
        l1 = pts2line(*s1)
        l2 = pts2line(*s2)
...
class Poly:
    def __init__(self, corners):
        ...
        self.xmin = min(corners)[0]
        self.xmax = max(corners)[0]
        self.seg = []
        for i in range(len(corners)):
            p1 = corners[i]
            p2 = corners[(i + 1)%len(corners)]
            self.seg.append(p1, p2)

    def intersects(self, other):
        intersects_at = []
        for s1 in self.seg:
            for s2 in other.seg:
                res, pt = segment_intersect(s1, s2)
                if res:
                    
                    

#for varje polygon kolla om de overlappar, spara segmenten som ;verlappar.
#om de inte ;verlappar i x eller y led kan de inte ;verlappa
#
N = ni()
poly = []
for n in range(N):
